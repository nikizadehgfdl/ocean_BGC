...
//             the py_script has import torch
  ...
  PyObject *varname_py = PyUnicode_FromString(varname_);
  PyObject *pValue;
  assert(pValue = PyObject_CallFunctionObjArgs(py_function, x1_py, x2_py, varname_py, NULL));
  if (pValue != NULL) {
       printf("Result of call: %f\n", PyFloat_AsDouble(pValue));
       Py_DECREF(pValue);
  }
  Py_DECREF(varname_py);
...
static void pyC_1_2d(const char *py_script_, const char *py_function_,
			const char *varname_,
			double *x, const int &n1, const int &n2)
{
  if(!Py_IsInitialized()) Py_Initialize();
  ...
  assert(init_numpy());
  _import_array(); //This function is not called by init_numpy() causing seg.fault with invalid memory error
  printf("pyC_1_2d: varname %s , dims %d,%d \n", varname_, n1,n2);  
  printf("From pyC_1_2d: Trying to call %s in %s.py \n",py_function_,py_script_);
  if(!py_module) {
    PyObject *py_script = PyUnicode_DecodeFSDefault(py_script_);
    assert(py_script);
    py_module = PyImport_Import(py_script);
...
//             the py_script has import torch
  ...
  //Simle tests
  //assert(PyObject_CallFunctionObjArgs(py_function, NULL)); //This worked. Printed from the function.
  const npy_intp dim_x [] = { n1,n2 };
  PyObject *x_py = PyArray_SimpleNewFromData(2, dim_x, NPY_DOUBLE, x);
  //printf("finished PyArray_SimpleNewFromData\n");
  PyObject *pValue;
  assert(pValue = PyObject_CallFunctionObjArgs(py_function, x_py, NULL));
  if (pValue != NULL) {
...
extern "C" void FORT(pyc_1array2d) (const char *py_script, const int &len_py_script,
		  const char *py_function, const int &len_py_function,
		  const char *varname, const int &len_varname,
		  double *x, const int &n1, const int &n2)

{
  ...
  pyC_1_2d(py_script_, py_function_, varname_, x, n1,n2);
